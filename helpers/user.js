...
//EDIT USERS 
exports.editAllUser = (obj) => {
    return new Promise((resolve, reject) => {
        db.query("UPDATE f_users SET ?", obj, (err, user) => {
            if (err) reject(err)
            else resolve(user)
        })
    })
...
//EDIT MULTIPLE USER
exports.editUsersThatCanEarn = (obj) => {
    return new Promise((resolve, reject) => {
        db.query("UPDATE f_users SET ? WHERE can_earn = 1", obj,  (err, user) => {
            if (err) reject(err)
            else resolve(user)
        })
    })
...
exports.deleteUnverfiedUser = () => {
    return new Promise((resolve, reject) => {
        db.query("DELETE FROM f_users WHERE is_verify = 0 AND is_paid = 0", (err, user) => {
            if (err) reject(err)
            else resolve(user)
        })
    })
...
exports.getUsersList = (search,limit,offset) => {
    return new Promise((resolve, reject) => {
        let query;
        if (search) {
            query = `SELECT * FROM f_users A WHERE (A.fullname LIKE '%${search}%' OR username LIKE '%${search}%') LIMIT ${limit} OFFSET ${offset}`
        } else {
